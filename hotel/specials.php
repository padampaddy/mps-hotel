 ...
 */
function call_specialsClass()
{
    new specialsClass();
}

if (is_admin()) {
    add_action('load-post.php',     'call_specialsClass');
    add_action('load-post-new.php', 'call_specialsClass');
}
...
class specialsClass
{
    public $name = 'mps_hotels_specials';
...
?>
        <a class="addimage button" onclick="custom_postimage_add_special('<?= $this->name ?>');">Add Image</a><br><br>
        <div id="<?= $this->name ?>_container" style="display:flex; gap:16px; overflow-x: scroll;">
            ...
            ?>
                <div id="<?= $this->name ?>_wrapper_<?= $i ?>" style="margin-bottom:20px;">
                    ...
                    <a class="removeimage" style="color:#a00;cursor:pointer;display:block" onclick="custom_postimage_remove_special('<?= $this->name ?>',<?= $i ?>);">Remove Image</a>
                </div>
            <?php } ?>
        </div>
        <input type="hidden" name="<?= $this->name ?>" id="<?= $this->name ?>" value="<?= implode(",", $values); ?>" />
        <script>
            function custom_postimage_add_special(key) {
            ...
            function custom_postimage_remove_special(key, index) {
